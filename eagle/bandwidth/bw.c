        ...
        while(non_handled>0){
            ...
            if(label[myrank] == 1){
                ...
                for(int i=0; i< trips; i++){
                    MPI_Send(arr, 64*1024  , MPI_CHAR, (myrank + shift) % size, 99, MPI_COMM_WORLD);
                    MPI_Send(arr, 512*1024 , MPI_CHAR, (myrank + shift) % size, 99, MPI_COMM_WORLD);
                    MPI_Send(arr, 1024*1024 , MPI_CHAR, (myrank + shift) % size, 99, MPI_COMM_WORLD);
                }
            }else if(label[myrank] == 50 || label[myrank] == 100){
                // will recieve data in this layer
                for(int i=0; i< trips; i++){
                    MPI_Recv(arr, 64*1024 , MPI_CHAR, (myrank - shift + size) % size, 99, MPI_COMM_WORLD, &status);
                    MPI_Recv(arr, 512*1024 , MPI_CHAR, (myrank - shift + size) % size, 99, MPI_COMM_WORLD, &status);
                    MPI_Recv(arr, 1024*1024 , MPI_CHAR, (myrank - shift + size) % size, 99, MPI_COMM_WORLD, &status);
                }
            }            
